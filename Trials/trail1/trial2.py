...
class UserDetailsForm(QDialog):
    def __init__(self, username):
        ...
        self.username = QLineEdit(user[0], readOnly=True)
        self.email = QLineEdit(user[1])
    ...
    def update(self):
        ...
        conn = sqlite3.connect('user_feedback.db')
        c = conn.cursor()
        c.execute('UPDATE users SET email = ?, password = ? WHERE username = ?', (email, password, self.username.text()))
        conn.commit()
